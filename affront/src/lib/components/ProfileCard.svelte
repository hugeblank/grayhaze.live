    ...
    <div class="flex flex-col md:flex-row m-2 p-3 bg-black bg-opacity-80 rounded-lg">
        <div class="flex flex-row md:flex-col items-start justify-center max-w-96">
            <div class="flex items-center">
                <div class="size-16 rounded-full overflow-hidden m-2 flex flex-grow items-center justify-center" style="background-image: url({ validator.avatar.value ? extractImage(validator.avatar.value) : "/nanashi.png" }); background-size: cover; background-position: center;">
                    {#if editable}
                        <label for="upload-avatar" class="size-full">
                ...
                <div>
                    ...
                    {:else}
                        <h5 class="content-around">{validator.displayName.value}</h5>
                    {/if}
                    {#if focus.handle}
            ...
            <div class="flex flex-col md:flex-row w-full mt-1">
                {#if editable}
                    ...
                    <label for="upload-banner" class="h-10 m-2 w-24 md:w-1/3">
                    ...
                    <button class="h-10 m-2 p-3 w-24 md:w-1/3 rounded-lg bg-blue-500 hover:bg-blue-700 text-neutral-100 hover:text-neutral-400 flex items-center justify-center" title="Import from BlueSky" aria-label="Import from BlueSky">
                        <i class="text-inherit bi bi-download"></i>&nbsp;<i class="text-inherit bi bi-moon-stars"></i>
                    </button>
                {:else if self && !edit}
                    <button onclick={() => edit = !edit} class="h-10 m-2 p-3 w-24 md:w-1/3 rounded-lg bg-green-600 hover:bg-green-800 text-neutral-100 hover:text-neutral-400 flex items-center justify-center" title="Edit Channel" aria-label="Edit Channel">
